...
function Services() {
  const services = [
    {
      title: "Henkilökuvaus",
      text: "Aidot ja tunnelmalliset henkilökuvat studiossa tai luonnossa.",
      img: "/gallery/henkilokuvaus/henk1.jpg",
      tab: "henkilokuvaus",
    },
    {
      title: "Tuotekuvaus",
  ...
  return (
    <section className="max-w-[1800px] mx-auto px-2 py-20 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 text-white">
      {services.map((s, i) => (
        <Link
          to={`/gallery?tab=${s.tab}`}
          key={i}
          className="block w-full max-w-[480px] mx-auto rounded-xl overflow-hidden bg-white/5 border border-white/10 hover:scale-105 hover:shadow-2xl transition-transform duration-300"
        >
